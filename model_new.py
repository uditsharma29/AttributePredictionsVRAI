# -*- coding: utf-8 -*-
"""
Created on Mon Sep 28 15:48:22 2020
...
# 			nn.Linear(in_features=512, out_features=256),
# 			nn.Linear(in_features=256, out_features=n_type_classes)
        )
...
# 			nn.Linear(in_features=256, out_features=n_color_classes)
		...
		self.sky = nn.Sequential(
            nn.Dropout(p=0.3),
            nn.Linear(in_features=2048, out_features = n_sky_classes),
			nn.Sigmoid()
# 			nn.Linear(in_features=512, out_features=256),
# 			nn.Linear(in_features=256, out_features=n_color_classes)
        )
...
# 			nn.Linear(in_features=256, out_features=n_color_classes)
	...
	def get_loss(self, net_output, ground_truth):
		#print("Ground truth: ",  ground_truth['wheel_labels'].type())
		#print("Predictions : ", net_output['wheel'].type())
		...
		#wheel_loss = F.cross_entropy(net_output['wheel'], ground_truth['wheel_labels'])
		luggage_loss = F.cross_entropy(net_output['luggage'], ground_truth['luggage_labels'])
		sky_loss = F.cross_entropy(net_output['sky'], ground_truth['sky_labels'])
		bumper_loss = F.cross_entropy(net_output['bumper'], ground_truth['bumper_labels'])
        #wheel_loss = CB_loss(samples_per_cls, num_classes, cfg.LOSS.TYPE, cfg.LOSS.BETA)
		#print(color_loss)
		loss = color_loss + type_loss + wheel_loss + luggage_loss + sky_loss + bumper_loss
		#print(loss)
		return loss, {'color': color_loss, 'type': type_loss, 'wheel': wheel_loss, 'luggage': luggage_loss, 
				'sky': sky_loss, 'bumper': bumper_loss}
